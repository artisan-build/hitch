...
// and actually spawned by writing its argv to a marker file the test can check.
...
function runShim(baseUrl, workDir, args = [], timeoutMs = 30000) {
...
test('forwards subcommands and defaults other arguments to install', (t) =>
  withCase(t, async (workDir) => {
    const { bytes, digest } = buildFixture(workDir);
    const server = await startServer({
      [`/dl/${asset}`]: (req, res) => {
    ...
    const base = `http://127.0.0.1:${server.address().port}/dl`;
    const cases = [
      { name: 'install', args: ['install', 'https://example.com/mcp'], want: ['install', 'https://example.com/mcp'] },
      { name: 'list', args: ['list'], want: ['list'] },
      { name: 'prompt', args: ['prompt', 'codex'], want: ['prompt', 'codex'] },
      { name: 'scan', args: ['scan', 'example'], want: ['scan', 'example'] },
      { name: 'uninstall with ordered flags', args: ['uninstall', '--yes'], want: ['uninstall', '--yes'] },
      { name: 'version', args: ['version'], want: ['version'] },
      {
        name: 'bare URL',
        args: ['https://example.com/mcp', '--name', 'example', '--client', 'cursor'],
        want: ['install', 'https://example.com/mcp', '--name', 'example', '--client', 'cursor'],
      },
      { name: 'unknown version flag', args: ['--version'], want: ['install', '--version'] },
    ];

    for (const testCase of cases) {
      await t.test(testCase.name, async () => {
        const caseDir = fs.mkdtempSync(path.join(workDir, 'case-'));
        const { result, marker } = await runShim(base, caseDir, testCase.args);
        assert.strictEqual(result.status, 0, `shim exited ${result.status}: ${result.stderr}`);
        const actual = fs.readFileSync(marker, 'utf8').trimEnd().split('\n');
        assert.deepStrictEqual(actual, testCase.want);
      });
    }
  }));
